    ...
    return {
        ...
        restart: () => set((state) => {
            if(state.phase === 'end' && state.status === 'win') {
                return { phase: 'lobby', blocksSeed: Math.random(), countArmy: 1, status: null, blocksCount: state.blocksCount + 20, maxSpeed: state.maxSpeed + 2, speedIncreaseRate: state.speedIncreaseRate + 0.1 }
            }
