...
export default function Ecctrl({
  ...
  const walkAnimation = !animated ? null : useAnim(state => state.walk)
  const runAnimation = !animated ? null : useAnim(state => state.run)
  const jumpAnimation = !animated ? null : useAnim(state => state.jump)
  const jumpIdleAnimation = !animated ? null : useAnim(state => state.jumpIdle)
  const jumpLandAnimation = !animated ? null : useAnim(state => state.jumpLand)
  const fallAnimation = !animated ? null : useAnim(state => state.fall)
  const action1Animation = !animated ? null : useAnim(state => state.action1)
  const action2Animation = !animated ? null : useAnim(state => state.action2)
  const action3Animation = !animated ? null : useAnim(state => state.action3)
  const action4Animation = !animated ? null : useAnim(state => state.action4)
  ...
  const moveCharacter = (_, run, slopeAngle, movingObjectVelocity) => {
    ...
    const angleBetweenCharacterDirAndObjectDir = movingObjectVelocity.angleTo(
      movingDirection
    )
        ...
        slopeAngle === null || slopeAngle == 0 // if it's on a slope, apply extra up/down force to the body
          ...
          : movingDirection.y *
              ...
              (movingDirection.y > 0 // check it is on slope up or slope down
                ? slopeUpExtraForce
                : slopeDownExtraForce) *
              (run ? sprintMult : 1), // if it's in the air, give it less control
        moveForceNeeded.z *
          turnVelMultiplier *
          (canJump ? 1 : airDragMultiplier)
      )
    ...
    >
      ...
      <group ref={characterModelRef} userData={{ camExcludeCollision: true }}>
        ...
        <mesh
          ...
          // this won't be collide by camera ray
